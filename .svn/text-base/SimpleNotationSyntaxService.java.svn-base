...
public class SimpleNotationSyntaxService implements INotationSyntaxService {
...
//		if(newP.isVisualisable())newP.setAppearance(prop.getAppearance());
 ...
 //   if(newP.isVisualisable())newP.setAppearance(prop.getAppearance());
	...
	public SimpleNotationSyntaxService(INotationSubsystem serviceProvider) {
	...
	createActivation();
	this.Catalusis = new LinkObjectType(this, 23, "Catalusis");
	createCatalusis();
	...
	//link set
		...
		this.links.put(this.Catalusis.getUniqueId(), this.Catalusis);
  ...
  public int numObjectTypes(){
	...
	this.Produce.getDefaultAttributes().setName("Consumption Link");
	...
	tport.getDefaultAttributes().setEndDecoratorType(LinkEndDecoratorShape.TRIANGLE);//, 20,20);
	tport.getDefaultAttributes().setEndSize(new Size(20,20));
	tport.getDefaultAttributes().setTermDecoratorType(PrimitiveShapeType.RECTANGLE);
	tport.getDefaultAttributes().setTermSize(new Size(0,0));
	int[] ctport=new int[]{255,255,255};
	tport.getDefaultAttributes().setTermColour(new RGB(ctport[0],ctport[1],ctport[2]));
	...
	Set<IShapeObjectType> set=null;
	int[] lc=new int[]{0,0,0};
	this.Catalusis.getDefaultAttributes().setLineWidth(1);
	this.Catalusis.getDefaultAttributes().setLineStyle(LineStyle.SOLID);
	this.Catalusis.getDefaultAttributes().setLineColour(new RGB(lc[0],lc[1],lc[2]));
	this.Catalusis.getDefaultAttributes().setName("Catalusis Link");
	this.Catalusis.getDefaultAttributes().setDescription("");
	this.Catalusis.getDefaultAttributes().setDetailedDescription("");
	this.Catalusis.getDefaultAttributes().setRouter(ConnectionRouter.SHORTEST_PATH);
	EnumSet<LinkEditableAttributes> editableAttributes = EnumSet.noneOf(LinkEditableAttributes.class);
	if(true){
	  editableAttributes.add(LinkEditableAttributes.COLOUR);
	}
	//this.Catalusis.getDefaultAttributes().setLineColourEditable(true);
	if(true){
	  editableAttributes.add(LinkEditableAttributes.LINE_STYLE);
	}
	//this.Catalusis.getDefaultAttributes().setLineStyleEditable(true);
	if(true){
	  editableAttributes.add(LinkEditableAttributes.LINE_WIDTH);
	}
	//this.Catalusis.getDefaultAttributes().setLineWidthEditable(true);
	this.Catalusis.setEditableAttributes(editableAttributes);

	this.Catalusis.getDefaultAttributes().setUrl("");
	//LinkEndDefinition sport=this.Catalusis.getLinkSource();
	//LinkEndDefinition tport=this.Catalusis.getLinkTarget();
	LinkTerminusDefinition sport=this.Catalusis.getSourceTerminusDefinition();
	LinkTerminusDefinition tport=this.Catalusis.getTargetTerminusDefinition();
	sport.getDefaultAttributes().setGap((short)2);
	sport.getDefaultAttributes().setEndDecoratorType(LinkEndDecoratorShape.NONE);//, 8,8);
	sport.getDefaultAttributes().setEndSize(new Size(8,8));
	sport.getDefaultAttributes().setTermDecoratorType(PrimitiveShapeType.RECTANGLE);
	...
	public LinkObjectType getCatalusis(){
	...
	private IPropertyDefinition getPropCID(){
		IPropertyDefinition CID=new PlainTextPropertyDefinition("CID"," ",true,true);
		return CID;
	}
